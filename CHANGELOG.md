...
- **Output token limit enforcement for Copilot models** — the proxy now reads `max_output_tokens` from the Copilot model registry and clamps `max_tokens` (or `max_completion_tokens` / `max_output_tokens` for OpenAI and Responses API formats) to the model's output token limit before forwarding. This prevents requests from exceeding backend output limits (e.g. Claude models have a 32k output cap). The clamping applies independently of the existing context-window guard and works across all three API paths (`/v1/messages`, `/v1/chat/completions`, `/v1/responses`).
...
- **Non-interactive auth fails fast instead of hanging** — when running as a system service (no TTY), the proxy now raises immediately with a clear error message if no GitHub token is available, instead of attempting the interactive OAuth device flow and blocking forever. The error message instructs the user to either set the `GITHUB_TOKEN` environment variable or run `claudegate` interactively once to persist a token.
- **Copilot timeouts now trigger fallback instead of returning 500** — `httpx.ReadTimeout` and other `httpx.TimeoutException` errors during the connection/stream-open phase were falling through to the generic exception handler and returning a 500 Internal Server Error. They are now caught in `_post_with_retry` and `_open_stream_with_retry` and converted to `TransientBackendError(504)`, which triggers the normal fallback-to-Bedrock path when a fallback backend is configured. When no fallback is configured, a clean 504 with "timeout_error" type is returned instead of a raw stacktrace.
...
- **OS trust store support** — use the native OS certificate store (macOS Keychain, Windows CertStore, Linux OpenSSL) via `truststore` so enterprise SSL inspection certificates are trusted automatically without manual cert installation.
- **Claude Code plugin** — `/models` skill available as a plugin. Install with `/plugin marketplace add https://github.com/Avi-141/gateway-to-ai.git` then `/plugin install models@claudegate`.
