...
def _is_model_not_supported(status_code: int, detail: str) -> bool:
    """Check if a Copilot error indicates the model is not supported on this endpoint."""
    return status_code == 400 and "model_not_supported" in detail
...
class CopilotBackend:
    ...
    async def handle_messages(
        ...
        try:
            return await self._handle_messages_via_chat(
                body, request_id, stream, openai_model, anthropic_model,
                client_context_window, initiator, log_prefix,
            )
        except CopilotHttpError as e:
            if _is_model_not_supported(e.status_code, e.detail):
                logger.warning(
                    f"{log_prefix}{openai_model} not supported on /chat/completions, "
                    "retrying via /responses"
                )
                return await self.handle_responses_messages(
                    body, request_id, stream, openai_model, anthropic_model,
                    client_context_window, initiator=initiator,
                )
            raise

    async def _handle_messages_via_chat(
        self,
        body: dict[str, Any],
        request_id: str,
        stream: bool,
        ...
        """Send a messages request via /chat/completions.

        Extracted to allow handle_messages() to retry via /responses on model_not_supported.
        """
    ...
    async def handle_openai_messages(
        ...
        try:
            return await self._handle_openai_messages_via_chat(
                openai_body, request_id, stream, copilot_model, log_prefix,
            )
        except CopilotHttpError as e:
            if _is_model_not_supported(e.status_code, e.detail):
                logger.warning(
                    f"{log_prefix}{copilot_model} not supported on /chat/completions, "
                    "retrying via /responses"
                )
                return await self.handle_openai_responses_messages(
                    openai_body, request_id, stream, copilot_model,
                )
            raise

    async def _handle_openai_messages_via_chat(
        self,
        ...
        """Send an OpenAI-format request via /chat/completions.

        Extracted to allow handle_openai_messages() to retry via /responses on model_not_supported.
        """
