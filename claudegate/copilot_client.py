...
class CopilotBackend:
    ...
    async def _open_messages_stream(
        ...
        Uses the pre-built *headers* (which include extra_headers like
        anthropic-beta) rather than rebuilding them on each retry.
        """
        ...
        for attempt in range(1 + self._retry_max):
            ...
            stream_cm = self._retry_client.stream("POST", COPILOT_MESSAGES_URL, headers=headers, json=body)
            try:
                resp = await stream_cm.__aenter__()
            except httpx.TimeoutException as exc:
                if attempt < self._retry_max:
                    ...
                    logger.warning(
                        f"{log_prefix}Copilot Messages stream timed out, "
                        f"retry {attempt + 1}/{self._retry_max} after {delay:.1f}s"
                    )
                    await asyncio.sleep(delay)
                    continue
                ...
                if attempt < self._retry_max:
                    delay = min(self._retry_base_delay * (2**attempt), 30.0)
                    logger.warning(
                        f"{log_prefix}Copilot Messages stream connection failed, "
                        f"retry {attempt + 1}/{self._retry_max} after {delay:.1f}s"
                    )
                    await asyncio.sleep(delay)
                    continue
            ...
            if resp.status_code == 429 and attempt < self._retry_max:
                ...
                await stream_cm.__aexit__(None, None, None)
                retry_after = _parse_retry_after(resp.headers)
                delay = min(retry_after or (self._retry_base_delay * (2**attempt)), 30.0)
                logger.warning(
            ...
            if resp.status_code >= 500 and attempt < self._retry_max:
                ...
                delay = min(retry_after or (self._retry_base_delay * (2**attempt)), 30.0)
                logger.warning(
                    f"{log_prefix}Copilot Messages stream {resp.status_code}, "
                    f"retry {attempt + 1}/{self._retry_max} after {delay:.1f}s"
                )
                await asyncio.sleep(delay)
                continue

            # Non-200 that isn't retryable
            if resp.status_code != 200:
