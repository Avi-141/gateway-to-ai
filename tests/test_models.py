...
class TestModelSupportsMessagesApi:
    ...
    def test_claude_heuristic_fallback_no_endpoints(self):
        """Claude model with no endpoint metadata falls back to heuristic (True)."""
        set_copilot_models([{"id": "claude-sonnet-4.5"}])
        assert model_supports_messages_api("claude-sonnet-4.5") is True

    def test_non_claude_heuristic_fallback_no_endpoints(self):
        """Non-Claude model with no endpoint metadata returns False via heuristic."""
        set_copilot_models([{"id": "gpt-4o"}])
        assert model_supports_messages_api("gpt-4o") is False

    def test_unknown_model_not_in_registry(self):
        """Model not in registry at all: Claude names use heuristic, others return False."""
        set_copilot_models([{"id": "some-other-model"}])
        assert model_supports_messages_api("claude-opus-4.6") is True
        assert model_supports_messages_api("gpt-5.4") is False

    def test_empty_registry_claude_heuristic(self):
        """Empty registry: Claude model names get heuristic True."""
        assert model_supports_messages_api("claude-sonnet-4.5") is True

    def test_empty_registry_non_claude(self):
        """Empty registry: non-Claude model names return False."""
        assert model_supports_messages_api("gpt-4o") is False
